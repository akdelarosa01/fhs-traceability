...
class BoxNGReasonController extends Controller
{
    ...
    {
        $data = [
			'msg' => 'Deleting Box N.G. Reason has failed.',
            'data' => [],
			'success' => true,
            'msgType' => 'warning',
            'msgTitle' => 'Failed!'
        ...
        try {
            if (is_array($req->ids)) {
                $update = PalletBoxNgReason::whereIn('id',$req->ids)
                            ->update([
                                'is_deleted' => 1,
                ...
                if ($update) {
                    $data = [
                        'msg' => "Box N.G. Reason was successfully deleted.",
                        'data' => [],
                        'success' => true,
                        'msgType' => 'success',
                        'msgTitle' => 'Success!'
                ...
                $update = PalletBoxNgReason::where('id',$req->ids)
                            ->update([
                                'is_deleted' => 1,
                                'update_user' => Auth::user()->id,
                                'updated_at' => date('Y-m-d H:i:s')
